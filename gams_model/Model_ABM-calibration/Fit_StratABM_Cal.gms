...
*number of geographical units that shall be calculated: max card(gn) = 377
 ...
 Sets
  ...
  tt                   "time period, used for yield distribution" /t2009/
 ;
 ...
 Parameters
  ...
  p_attYield(gn,tt)                       "attainable yield"
...
* ---Reload Variables and Parameters from steps 1- 4 into programme
  execute_loadpoint "ResultBetaEstimate.gdx"
     ...
     p_attYield,p_yieldLossD0,p_CRate,p_yieldLossD0,p_yieldLossDinfin,
     p_soilType,p_affYieldShare,p_processes,
     p_TDistribution,p_weatherStation, p_yieldMgm
  ;


* ---Define Parameters for part (5):
  Parameter
     p_attyield(gn,tt)                              "attainable yield for GM calculations: water-limited potential yield + 5%"
     p_costsVar(gn,tt,spre,spost)                   "total variable costs dependend on weed control strategy"
     p_costsFix(gn,tt,spre,spost)                   "total fixed costs dependend on weed control strategy"
     p_costsInterest(gn,tt,spre,spost)              "Interest claim"
     p_cropPrice                                    "Price of silage maize" /4.00/
     ...
     p_fertNneed(gn,tt,spre,spost)                  "necessary N-fertiliser"
     p_fertPneed(gn,tt,spre,spost)                  "necessary P-fertiliser"
     p_fertKneed(gn,tt,spre,spost)                  "necessary K-fertiliser"
     p_fertneedtot(gn,tt,spre,spost)                "total necessary fertiliser"
     p_fertapplcost(gn,tt,spre,spost)               "fertiliser application cost depended on yield and amount of fertiliser"
     p_GLYUSER(gn,tt)                               "glyphosate user yes/no, No Risk"
     p_gm(gn,tt,spre,spost)                         "possibility matrix of gross margin for municipality gn for strategy combinations spre and spost"
     ...
     p_nutrNyield(gn,tt,spre,spost)                 "N in achieved yield"
     p_nutrPyield(gn,tt,spre,spost)                 "P in achieved yield"
     p_nutrKyield(gn,tt,spre,spost)                 "K in achieved yield, No Risk"
     p_postSowDamage(gn,tt,spost,s_process)         "post-sow damage with estimated parameters"
...
*
 p_attyield(gn,tt) =  p_yieldMgm(gn,tt) * (1 + p_affYieldShare);


*
* ---Use betas and create production function (possibly with changed p_timeofEmergence2)
*
 p_postSowDamage(gn,tt,spost,s_process) =   {1 -  p_yieldLossD0 * v_Dpost.l(gn,spost)
                                              / [100 * ( exp(p_CRate * p_TDistribution(gn,tt,s_process) )
                                                     + (p_yieldLossD0 * v_Dpost.l(gn,spost) ) / p_yieldLossDinfin ) ] };


  p_yieldFunc(gn,tt,spre,spost)  = [1 - EXP( -  sqr ( v_beta.l('0') + v_beta.l('1') * p_v (gn,spre) ) ) ]
                                    *  p_attYield(gn,tt)
                                    *  sum[ s_process, p_postSowDamage(gn,tt,spost,s_process) * p_processes(spre,s_process) ] ;
...
*
  p_nutrNyield(gn,tt,spre,spost) = p_yieldFunc(gn,tt,spre,spost) * p_NPKshare('N');
  p_nutrPyield(gn,tt,spre,spost) = p_yieldFunc(gn,tt,spre,spost) * p_NPKshare('P');
  p_nutrKyield(gn,tt,spre,spost) = p_yieldFunc(gn,tt,spre,spost) * p_NPKshare('K');

  p_fertNneed(gn,tt,spre,spost)  = p_nutrNyield(gn,tt,spre,spost) - p_manure * p_NPKcont ('N');
  p_fertPneed(gn,tt,spre,spost)  = p_nutrPyield(gn,tt,spre,spost) - p_manure * p_NPKcont ('P');
  p_fertKneed(gn,tt,spre,spost)  = p_nutrKyield(gn,tt,spre,spost) - p_manure * p_NPKcont ('K');

  p_fertneedtot(gn,tt,spre,spost) = p_fertNneed(gn,tt,spre,spost)/p_weightshare('N')
                                  + p_fertPneed(gn,tt,spre,spost)/p_weightshare('P')
                                  + p_fertKneed(gn,tt,spre,spost)/p_weightshare('K');
...
*
;
  p_fertapplcost(gn,tt,spre,spost)
        = sum(  s_fertAmount $ (      (p_fertneedtot(gn,tt,spre,spost) ge p_TotFert(s_fertAmount-1))
                                  and (p_fertneedtot(gn,tt,spre,spost) <  p_TotFert(s_fertAmount))),
...
* ---Rating is only done every 5th year
      ...
      +  p_seedcost
      +  p_fertapplcost (gn, tt,spre,spost)
      +  p_fertNneed (gn,tt,spre,spost) * p_NPKprice ('N')
      +  p_fertPneed (gn,tt,spre,spost) * p_NPKprice ('P')
      +  p_fertKneed (gn,tt,spre,spost) * p_NPKprice ('K')
...
*       ---Costs for harvest:
      +  [(86.89 + 0.0009 * p_yieldFunc(gn,tt,spre,spost))   + (31.838 + 0.01948 * p_yieldFunc(gn,tt,spre,spost))      + ((0.4325 + 0.00003 * p_yieldFunc(gn,tt,spre,spost)) * p_wage('workhour'))]
*       ---Costs for transport:
      +  [(4.8075 + 0.07933 * p_yieldFunc(gn,tt,spre,spost)) + (8.36 + 0.0594 * p_yieldFunc(gn,tt,spre,spost))         + ((0.7425 + 0.00308 * p_yieldFunc(gn,tt,spre,spost)) * p_wage('workhour'))]
*       ---Costs for compaction:
      +  [(0.005 + 0.02885 * p_yieldFunc(gn,tt,spre,spost))  + (0.005 + 0.01985 * p_yieldFunc(gn,tt,spre,spost))       + ((0.2225 + 0.00218 * p_yieldFunc(gn,tt,spre,spost)) * p_wage('workhour'))]
  ;

*
* --- Matrix of gross margins for each strategy
*
  p_revenue(gn,tt,spre,spost) = p_yieldFunc(gn,tt,spre,spost) * p_cropPrice;
  p_costsInterest(gn,tt,spre,spost) = p_interest * 6/12 * [p_costsVar(gn,tt,spre,spost) + p_HerbRelCosts (spre,'machcostsFix') + p_HerbRelCosts (spost,'machcostsFix')];
  p_gm(gn,tt,spre,spost) = p_revenue(gn,tt,spre,spost) - p_costsVar(gn,tt,spre,spost) - p_HerbRelCosts (spre,'machcostsFix') - p_HerbRelCosts (spost,'machcostsFix') - p_costsInterest(gn,tt,spre,spost);
...
* --------------------------------------------------------------------------
  ...
  Equations
     e_gm(gn,tt)              "gross margin calculation for municipality gn for strategy cominations spre and spost"
     e_gmobj                  "total gross margin in model"
     e_IPre(gn,tt)            "sum of pre-sowing strategies has to be unity for each gn"
     e_IPost(gn,tt)           "sum of post-sowing strategies has to be unity for each gn"
     e_NicoRestr(gn,tt)       "restriction for nicosulfuron-containing plant protection products"
     e_HRAC(gn,s_HRAC,tt)     "restriction that HRAC categories have to be changed"

  ;

  Variables
     v_gm(gn,tt)              "gross margin for municipality gn for strategy cominations spre and spost"
     v_gmobj                  "total gross margin for all gn"
     v_shareSpray(s,gn,tt)    "variable to chose a strategy"
  ;

*
* --- Bounds and starting values for v_shareSpray
*
  v_shareSpray.lo(s,gn,tt) = 0;
  v_shareSpray.up(s,gn,tt) = 1;
$ontext
...
$offtext
  v_shareSpray.l('spre1',gn,'t2009') = 1;
  v_shareSpray.l('spost1',gn,'t2009') = 1;
$ontext
...
*
  e_IPre(gnCur(gn),tt) ..
    sum(spre, v_shareSpray(spre,gn,tt) )  =E= 1;

  e_IPost(gnCur(gn),tt) ..
    sum(spost, v_shareSpray(spost,gn,tt)) =E= 1;

*
* --- Nicosulfuron restriction (only usable every second year):
*
  e_NicoRestr(gnCur(gn),tt) ..
     Sum [(spre),  p_asToS(spre,'Nicosulfuron')  * v_shareSpray(spre,gn,tt)]
   + Sum [(spost), p_asToS(spost,'Nicosulfuron') * v_shareSpray(spost,gn,tt)]

   + Sum [(spre),  p_asToS(spre,'Nicosulfuron')  * v_shareSpray(spre,gn,tt+1)]
   + Sum [(spost), p_asToS(spost,'Nicosulfuron') * v_shareSpray(spost,gn,tt+1)]

   =L= 1;

*
* --- HRAC restriction -> Farmer has to change HRAC categories of pesticides every second year:
*
  e_HRAC(gnCur(gn),s_HRAC,tt)..
     Sum[ spre, p_HRAC(spre,s_HRAC)  * v_shareSpray(spre,gn,tt)  ]
   + Sum[ spost, p_HRAC(spost,s_HRAC) * v_shareSpray(spost,gn,tt) ]

  + Sum[ spre, p_HRAC(spre,s_HRAC)  * v_shareSpray(spre,gn,tt+1)  ]
  + Sum[ spost, p_HRAC(spost,s_HRAC) * v_shareSpray(spost,gn,tt+1) ]
...
*
  e_gmobj ..
    v_gmobj =E= Sum[ (gnCur(gn),tt), v_gm(gn,tt) ]
  ;
...
$iftheni.GlyphosateBan %GlyphosateBan%==ON
    ...
    execute_loadpoint "Grossmargin_P4,00NoRisk.gdx"   p_GLYUSER;
$else.GlyphosateBan

   p_GLYUSER(gn,tt) = 1$(v_sharespray.l('spre5',gn,tt) > 0 or v_sharespray.l('spre6',gn,tt) > 0) ;
$endif.GlyphosateBan
...
* ----------------------------------------------------------------------------------------
  Parameters
     p_appSharePre(as,tt)                  "Application share of AS as calculated by model for pre-sowing strategies"
     p_appSharePost(as,tt)                 "Application share of AS as calculated by model for post-sowing strategies"
     ...
     p_appSharePostGLY(as,tt)              "only of glyphosate using m"
...
* --- Share of active substances and cultivations strategies for pre-sowing strategies
  ...
  p_appSharePre(as,tt) = sum( (spre,gn), v_shareSpray.l(spre,gn,tt) * p_asToS(spre,as)) / card(gn);

* --- Share of active substances and cultivations strategies for post-sowing strategies

  p_appSharePost(as,tt) = sum( (spost,gn), v_shareSpray.l(spost,gn,tt) * p_asToS(spost,as)) / card(gn) ;

* --- As above, but only of glyphosate users

  p_appSharePostGLY(as,tt) = sum( (spost,gn) $ (p_GLYUSER(gn,tt) > 0),
                                         v_shareSpray.l(spost,gn,tt) * p_asToS(spost,as)) /  card(gn) ;


* --- Share of active substances and cultivations strategies for pre-sowing strategies

  p_appSharePreGN(gn,as) = sum( (spre,tt), v_shareSpray.l(spre,gn,tt) * p_asToS(spre,as)) / card(tt);

* --- Share of active substances and cultivations strategies for post-sowing strategies

  p_appSharePostGN(gn,as) = sum( (spost,tt), v_shareSpray.l(spost,gn,tt) * p_asToS(spost,as)) / card(tt) ;
...
Parameter p_GMAverage(gn) "Average gross margin for each geographical unit";
 loop(gn1 $ (ord(gn1) le gn_inAnalysis),
      option kill=gnCur;gnCur(gn1)=yes;
      p_GMAverage(gnCur(gn)) = SUM[ tt, v_gm.l(gn,tt)/Card(tt)];
      );
Display p_GMAverage;
...
*  fh.nd=5;
  ...
  loop(gn1 $ (ord(gn1) le gn_inAnalysis),
      option kill=gnCur;gnCur(gn1)=yes;
     Loop ((gnCur(gn),spre,spost)$(v_sharespray.l(spre,gn,'t2009') and v_sharespray.l(spost,gn,'t2009')),
         Put gn.tl","
             p_GMAverage(gn)","
             spre.tl ","
             spost.tl ","
