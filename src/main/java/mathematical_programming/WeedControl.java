...
public class WeedControl implements MP_Interface{
	...
	public WeedControl(Properties cmd, int simYear, int memoryLengthAverage) throws IOException {
		...
		this.year_price = readMPyearPrice(simYear,memoryLengthAverage);             // Initialize MP inputs (edit number of agents, year and price)
		this.yearlyPrices = (List<Double>) this.year_price.get(1);
		this.listOfYears = (List<String>) this.year_price.get(0);
	}
    ...
    private void editMPscript(int nFarm, int year, boolean pricingAverage, int memoryLengthAverage) throws FileNotFoundException, IOException {	
		...
		Object MP_year = this.listOfYears.get(memoryLengthAverage + year-1);
		Object MP_price = this.yearlyPrices.get(memoryLengthAverage + year-1); 
		
		if (pricingAverage == true) {		
			List<Double> memoryPrice = new ArrayList<Double>();
			for(int i = memoryLengthAverage; i > 0; i--) {
				memoryPrice.add(this.yearlyPrices.get( memoryLengthAverage - i + (year - 1)));
			}
