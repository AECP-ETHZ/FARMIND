...
public class WeedControl implements MP_Interface{
	...
	private void createRunGamsBatch(Properties cmd, String OS) {
		//System.out.println(cmd.getProperty("debug"));
		if (cmd.getProperty("debug").equals("1")) {
			...
			try {
				...
				LOGGER.fine(proj + " gams Fit_StratABM_Cal");
				writer.close();
			} catch (IOException e) {
				e.printStackTrace();
			}
	...
	public List<Double> readMPIncomes(List<Farm> allFarms) {
		...
		List<Object> data = readMPOutputFiles(allFarms);			                   // read data file generated by MP
